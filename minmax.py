...
def limit_ev(house, i):
    if house.ev.session[i] == -1:  # vehicle not home, so minmax = [0,0]
        house.ev.minmax = [0, 0]
    else:
        session = int(house.ev.session[i])  # determine the ev charging session number
        # minimum power required to charge the EV to the "required energy" in the time where the vehicle is home
        required_energy = house.ev.size  # always charge to 100% SoC
        min_power = (
            max(0, (required_energy - house.ev.energy))
        ) * 4  # multiply by four because of conversion from kWh to kW
        time_left = (
            house.ev.session_leave[session] - i
        )  # determine how many timesteps are left before the vehicle leaves
        min_power = min(
            house.ev.power_max, (min_power / time_left)
        )  # determine the min power by dividing the required power by the number of timesteps left
        # max charge power possible
        energy_left = (
            house.ev.size - house.ev.energy
        ...
        house.ev.minmax = [min_power, max_power]  # store value in house


def limit_batt(house):
    dis_power = max(
        -(house.batt.energy * 4), -house.batt.power_max
    )  # determine maximum discharge power (either what is left or max discharge power), this value is negative (multiplication by 4 for conversion from energy to power)
    charge_power = min(
        (house.batt.size - house.batt.energy) * 4, house.batt.power_max
...
def limit_hp(house, i, T_ambient):
    ...
    house.heat_demand_house[
        i
    ] = heat_demand_house  # value needs to be stored for response.py
    house.hp.minmax = minmax


def limit_ders(list_of_houses, i, T_ambient):
    for house in list_of_houses:
        if house.ders[0] == 1:  # pv
            house.pv.minmax = [
                0,
                house.pv.data[i],
            ]  # the minimum is always 0 and the maximum is what the generation data gives
        if house.ders[1] == 1:  # ev
            limit_ev(house, i)
        if house.ders[2] == 1:  # batt
            limit_batt(house)
        if house.ders[3] == 1:  # hp
